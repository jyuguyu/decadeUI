...
export default async function () {
	...
	return {
								...
								$throwordered2(card, nosource) {
								    ...
								    if (1) {
								        ...
								        if (node && node.node) {
								            ...
								            game.broadcastAll((node, eventInfo, { name, number, suit }, event) => {
								                let card = node;
								                if (!node.node) {
		...
		package: (() => {
			...
			pack.intro = (pack => {
				let log = [`魔改十周年UI ${pack.version}`, "最低适配：v1.10.18 暂定", "修复手杀UI自动整理，UI显示问题", "添加私货meihua.js，神选势力交互音效等", "新增OLUI移动端，感谢群友提供的素材", "添加全选按钮 by咪咪狗奇妙工具", "注释指示线美化，需要自行解除"];
