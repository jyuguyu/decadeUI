		...
		click: {
			...
			playerIntro(e) {
				...
				container.show = function(player) {
					...
					biankuang3.setBackgroundImage(
						`extension/十周年UI/shoushaUI/character/images/宝宝杀/baby_${player.group}.png`);

					//势力
					var biankuang4 = ui.create.div(".biankuang4", blackBg1);
					biankuang4.setBackgroundImage(
						`extension/十周年UI/shoushaUI/character/images/宝宝杀/babys_${player.group}.png`);

					//通过势力判断技能框的背景颜色
					var extensionPath = lib.assetURL + "extension/十周年UI/shoushaUI/";
					var group = player.group;
					if (group != "wei" && group != "shu" && group != "wu" && group != "qun" && group !=
						"ye" && group != "jin" && group != "daqin" && group != "western" && group !=
						"shen" && group != "key" && group != "Han" && group != "qin") group = "default";
					...
					xingxing.setBackgroundImage("extension/十周年UI/shoushaUI/character/images/宝宝杀/" +
						rarity + ".png");
					...
					viewBusinessCard.onclick = function() {
						...
						var gonghui = ui.create.div(".gonghui", bigdialog, get.translation((
							innerText = "(" + (num = ["无名杀会员", "手机三国杀会员", "三国杀ol会员",
								"三国杀十周年会员", "怒焰三国杀会员", "欢乐三国杀会员", "阵面对决会员"
							]).randomGet(1) + ")")));
						var xianhua = ui.create.div(".xianhua", bigdialog, get.translation((
							innerText = "鲜花" + (num = Math.floor(Math.random() * (999 -
								1 + 1) + 1)))));
						var jidan = ui.create.div(".jidan", bigdialog, get.translation((innerText =
							"鸡蛋" + (num = Math.floor(Math.random() * (999 - 1 + 1) + 1))
							)));
						var fenxiang = ui.create.div(".fenxiang", bigdialog, get.translation((
							innerText = "分享")));
						var zhanshi = ui.create.div(".zhanshi", bigdialog, get.translation((
							innerText = "展示(诏令－1)")));

						//var shanchang = get.config('recentCharacter');
						var shanchang = ["sp_diaochan", "sp_zhaoyun", "sp_sunshangxiang",
							"sp_caoren", "sp_jiangwei", "sp_machao", "sp_caiwenji",
							"jsp_guanyu", "jsp_huangyueying", "sp_pangde", "sp_jiaxu",
							"yuanshu", "sp_zhangliao", "sp_ol_zhanghe", "wulan", "leitong",
							"huaman", "wangshuang", "wenyang", "re_liuzan", "caobuxing",
							"re_maliang", "xin_baosanniang", "re_xinxianying", "dongxie",
							"guozhao", "fanyufeng", "ruanyu", "liangxing", "re_dongzhao",
							"yangwan", "re_panshu", "dufuren", "zhouyi", "lvlingqi", "re_kanze",
							"caojinyu", "caocao", "simayi", "xiahoudun", "zhangliao", "xuzhu",
							"guojia", "zhenji", "liubei", "guanyu", "zhangfei", "zhugeliang",
							"zhaoyun", "machao", "huangyueying", "sunquan", "ganning", "lvmeng",
							"huanggai", "zhouyu", "daqiao", "luxun", "sunshangxiang", "huatuo",
							"lvbu", "diaochan"
						];
						var jingjitu = ["jingji1", "jingji2", "jingji3", "jingji4"];
						var xingbietu = ["xingbie1", "xingbie2"];

						shanchang1.setBackgroundImage("image/character/" + shanchang.randomGet() +
							".jpg");
						shanchang2.setBackgroundImage("image/character/" + shanchang.randomGet() +
							".jpg");
						shanchang3.setBackgroundImage("image/character/" + shanchang.randomGet() +
							".jpg");
						shanchang4.setBackgroundImage("image/character/" + shanchang.randomGet() +
							".jpg");
						...
						minixingxiang.style.backgroundImage = player.node.avatar.style
							.backgroundImage;
						jingji.setBackgroundImage("extension/十周年UI/shoushaUI/character/images/" +
							jingjitu.randomGet() + ".png");
						xingbie.setBackgroundImage("extension/十周年UI/shoushaUI/character/images/" +
							xingbietu.randomGet() + ".png");
					};
					...
					if (shownHs.length) {
						ui.create.div(".xcaption", player.getCards("h").some(card => !shownHs.includes(
							card)) ? "明置的手牌" : "手牌区域", rightPane.firstChild);
						...
						oSkills.forEach(function(name) {
							if (player.forbiddenSkills[name]) {
								if (player.forbiddenSkills[name].length) ui.create.div(
									...
									'<span style="opacity:0.5">' + lib.translate[name] +
									"</span>" + "</div>" + "<div>" +
									'<span style="opacity:0.5">' + "（与" + get.translation(
										player.forbiddenSkills[name]) + "冲突）" + get
								...
								if (lib.skill[name].preHidden && get.mode() == "guozhan") {
									...
									if (_status.prehidden_skills.includes(name)) underlinenode
										.classList.remove("on");
									...
									'<span style="opacity:0.5">' + lib.translate[name] +
									"</span>" + "</div>" + "<div>" +
									'<span style="opacity:0.5">' + get.skillInfoTranslation(
										name, player) + "</span>" + "</div>", rightPane
									.firstChild);
							} else if (!player.getSkills().includes(name) || player
								.awakenedSkills.includes(name)) ui.create.div(".xskill",
								"<div data-color>" + '<span style="opacity:0.5">' + lib
								.translate[name] + "</span>" + "</div>" + "<div>" +
								'<span style="opacity:0.5">' + get.skillInfoTranslation(
									name, player) + "</span>" + "</div>", rightPane
								.firstChild);
								...
								if (lib.skill[name].subfrequent) {
									for (var j = 0; j < lib.skill[name].subfrequent
										.length; j++) {
										if (lib.config.autoskilllist.includes(name + "_" + lib
												.skill[name].subfrequent[j])) {
											underlinenode.classList.remove("on");
										}
									}
								}
								...
								if (!_status.gameStarted || (lib.skill[name].clickableFilter &&
							...
							} else ui.create.div(".xskill", "<div data-color>" + lib.translate[
								name] + "</div>" + "<div>" + get.skillInfoTranslation(
								name, player) + "</div>", rightPane.firstChild);
						});
					}
					//装备*
					...
					if (eSkills.length) {
						...
						eSkills.forEach(function(card) {
							...
							if (cards?.length && (cards?.length !== 1 || cards[0].name !== card
									.name)) str[0] += "（" + get.translation(card.cards) + "）";
							const special = card.cards?.find(item => item.name == card.name &&
								lib.card[item.name]?.cardPrompt);
							if (special) str[1] = lib.card[special.name].cardPrompt(special,
								player);
							ui.create.div(".xskill", "<div data-color>" + str[0] +
					...
					if (judges.length) {
						...
						judges.forEach(function(card) {
							...
							ui.create.div(".xskill", "<div data-color>" + str + "</div><div>" +
								get.translation(card.name + "_info") + "</div>", rightPane
								.firstChild);
						});
					}
